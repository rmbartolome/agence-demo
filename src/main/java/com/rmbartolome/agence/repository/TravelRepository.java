...
public interface TravelRepository extends JpaRepository<Travel,Integer>{
    //List<Travel> findWorkerAndVehicle(Integer worker_id, Integer vehicle_id);

    @Query(value = "SELECT * FROM TRAVELS WHERE FINISH_DATE >= :startDate AND FINISH_DATE <= :endDate", nativeQuery = true)
    List<Travel> getAllBetweenDates(@Param("startDate") LocalDate startDate, @Param("endDate") LocalDate endDate);
}
